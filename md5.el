...
(defun md5-block-index (step-index round)
  (let ((mod-index (- step-index (* round 16))))
    (cond ((= round 0) mod-index)
          ((= round 1) (logand #xF (+ (* mod-index 5) 1)))
          ((= round 2) (logand #xF (+ (* mod-index 3) 5)))
          ((= round 3) (logand #xF  (* mod-index 7)))
...
(defun md5-apply-step (a0 b0 c0 d0 message)
  ...
  (let ((a a0)
    ...
    (dotimes (i 64)
      (let* ((round (/ i 16))
             (block-index   (md5-block-index i round))
             ...
             (f (md5-add (md5-fun i round b c d) a m md5-t))
             (s (elt md5-s-table i)))
        (setq a d)
        (setq d c)
        (setq c b)
...
(defun md5-pad (msg)
  (let* ((bytified-msg (string-to-unibyte msg))
    ...
    (let ((pad-zeroes (make-list (/ missing-bits 8) #x00)))
      ...
      (vconcat bytified-msg pad-zeroes bitcount-pad))))
